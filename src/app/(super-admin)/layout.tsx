...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
        ...
        const checkSession = async () => {
            ...
            if (sessionRaw) {
                ...
                try {
                    ...
                    if (!isPublicPage) router.replace("/super-admin/login");
    ...
    if (!isAuthorized && !isAuthPage) return <div className="min-h-screen bg-surface-light flex items-center justify-center"><div className="w-8 h-8 border-4 border-amber-600 border-t-transparent rounded-full animate-spin" /></div>;

    const isNavActive = (href: string) => {
        if (href === "/super-admin/partners") return pathname.startsWith("/super-admin/partners");
    ...
    const SidebarContent = () => (
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="px-6 pb-4">
                <div className="flex items-center gap-2 px-4 py-2 rounded-md border border-amber-200 bg-amber-50 text-amber-700 text-[10px] font-black uppercase tracking-widest">
            ...
            {!isAuthPage && (
                <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-md border-b border-gray-100 px-4 flex items-center justify-between z-40">
                    <Link href="/" className="flex items-center gap-2">
                        ...
                        <span className="font-black text-xl text-gray-900 tracking-tighter">Super Admin</span>
                    </Link>
                    <button 
